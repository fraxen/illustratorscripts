...
//
//Modified by Hugo Ahlenius, April 2012 with some dirty hacks to rotate objects, and to only scale widths.
//
//>=--------------------------------------
...
if (documents.length>0) {
	if (docRef.selection.length > 1) {
		...
		if (mySelection instanceof Array) {
			...
			var oSelRatio = oSelWidth / oSelHeight
			// *********************************************************
			var initBounds;
			var ul_x;
			var ul_y;
			...
			for (var i=0; i < mySelection.length; i++) {
				...
				mySelAngle = mySelection[i].fillColor.black * -3.6;
				eval('subArray' + i + '["angle"]=' + mySelAngle);
			...
			if ( scaledObject == true) {
				...
				if ( onlyScaleWidth == true ) {
					// Added by Hugo Ahlenius in April 2012
					eval('newGroup.height = alterObjectArray['+ i +']["width"] / ' + oSelRatio);
					shift = (newGroup.height - eval('alterObjectArray['+i+']["height"]')) / 2;
					newGroup.top = newGroup.top + shift;
				} else {
				...
				if( scalingCentered == true ) {
					eval('newGroup.position = [alterObjectArray['+i+']["offsetXpos"], alterObjectArray['+i+']["offsetYpos"] - shift]');
				} else {
					eval('newGroup.position = [alterObjectArray['+i+']["xpos"], alterObjectArray['+i+']["ypos"]]');		
				}
			}

			// If rotate toggle, rotate the replacing object
            if ( rotateObject == true ) {
				// Added by Hugo Ahlenius in April 2012
				// TODO: This should be neater, it should loop over all items in the group...
				try {
					eval('newGroup.pathItems[0].rotate(alterObjectArray[' + i +']["angle"])');
				}
				catch(err) {
